...
CREATE TABLE [dbo].[T_OE_USERS](
	...
	[NOTIFY_NEWSLETTER] [BIT] NOT NULL DEFAULT 1,
	[CREATE_USERIDX] [int] NULL,
	[CREATE_DT] [datetime2](0) NULL,
	[MODIFY_USERIDX] [int] NULL,
	[MODIFY_DT] [datetime2](0) NULL,
	[PROJECT_UPDATE_OPTOUT_IND] bit NOT NULL DEFAULT 0,
...
--ALTER TABLE [T_OE_USERS] ADD [NOTIFY_NEWSLETTER] [BIT] NOT NULL DEFAULT 1;
--ALTER TABLE [T_OE_USERS] ADD [NEW_USER_EMAIL_IND] [BIT] NOT NULL DEFAULT 0;
--ALTER TABLE [T_OE_USERS] add [PROJECT_UPDATE_OPTOUT_IND] [BIT] NOT NULL DEFAULT 0;
--update T_OE_USERS set NEW_USER_EMAIL_IND=1;
...
CREATE TABLE [dbo].[T_OE_PROJECTS](
    ...
    [PROJ_DESC_HTML] [varchar](max) NULL,
	[MEDIA_TAG] [int] NULL,
	[START_YEAR] [int] NULL,
	...
	[MODIFY_USERIDX] [int] NULL,
	[MODIFY_DT] [datetime2](0) NULL,
	[PROJECT_REMIND_DT] datetime2(0) NULL,
	[PROJECT_REMIND_FREQ] varchar(1) NULL default 'Q',
	[PROJECT_REMIND_CNT] int NOT NULL default 0,
...
--  alter table T_OE_PROJECTS add [PROJ_DESC_HTML] [varchar](max) NULL;
--  alter table T_OE_PROJECTS ADD PROJECT_REMIND_DT datetime2(0) NULL;
--  alter table T_OE_PROJECTS add PROJECT_REMIND_FREQ varchar(1) NULL default 'Q';
--  alter table T_OE_PROJECTS add PROJECT_REMIND_CNT int NOT NULL default 0;

CREATE TABLE [dbo].[T_OE_PROJECT_ORGS](
	[PROJECT_ORG_IDX] [uniqueidentifier] NOT NULL DEFAULT newid(),
	[PROJECT_IDX] [uniqueidentifier] NOT NULL,
	[ORG_IDX] [uniqueidentifier] NULL,
	[CREATE_USERIDX] [int] NULL,
	[CREATE_DT] [datetime2](0) NULL,
	[MODIFY_USERIDX] [int] NULL,
	[MODIFY_DT] [datetime2](0) NULL,
 CONSTRAINT [PK_T_OE_PROJECT_ORGS] PRIMARY KEY CLUSTERED  ([PROJECT_ORG_IDX] ASC),
 FOREIGN KEY ([ORG_IDX]) references [T_OE_ORGANIZATION] ([ORG_IDX])  ON UPDATE CASCADE ON DELETE CASCADE,
...
GO	 
--alter table [T_OE_PROJECT_URLS] alter column [PROJECT_URL] varchar(300) NULL;
...
CREATE TABLE [dbo].[T_OE_APP_SETTINGS_CUSTOM](
	...
	[WELCOME_EMAIL] [varchar](max) NULL,
	[REMINDER_EMAIL] [varchar](max) NULL,	
...
--  alter table [T_OE_APP_SETTINGS_CUSTOM] add [WELCOME_EMAIL] [varchar](max) NULL;
--  alter table [T_OE_APP_SETTINGS_CUSTOM] add [REMINDER_EMAIL] [varchar](max) NULL;
...
GO
 ...
 CREATE VIEW STALE_PROJECTS_WITH_CONTACTS
 as
 select P.PROJECT_IDX, P.PROJ_NAME, P.PROJ_STATUS, 
 (select top 1 O.ORG_NAME from T_OE_PROJECT_ORGS PO, T_OE_ORGANIZATION O where PO.ORG_IDX=O.ORG_IDX and PO.[PROJECT_IDX]=P.PROJECT_IDX) as ORG,
 case when P.PROJECT_REMIND_DT is null then UC.FNAME else UC1.FNAME end as FNAME, 
 case when P.PROJECT_REMIND_DT is null then UC.LNAME else UC1.LNAME end as LNAME, 
 case when P.PROJECT_REMIND_DT is null then UC.EMAIL else UC1.EMAIL end as EMAIL, 
 COALESCE(P.MODIFY_DT, P.CREATE_DT) as CREAT_UPDATE_DT, 
 DATEADD(d, 1, EOMONTH(P.PROJECT_REMIND_DT)) as PROJECT_REMIND_DT, 
 CASE WHEN COALESCE(P.MODIFY_DT, P.CREATE_DT) > coalesce(P.PROJECT_REMIND_DT,'1/1/1990') 
	  THEN COALESCE(P.MODIFY_DT, P.CREATE_DT) else P.PROJECT_REMIND_DT end as TRU_LAST_DT,
 CASE WHEN COALESCE(P.MODIFY_DT, P.CREATE_DT) > coalesce(P.PROJECT_REMIND_DT,'1/1/1990') 
	  THEN DATEADD(month, 6, COALESCE(P.MODIFY_DT, P.CREATE_DT)) else DATEADD(month,6,P.PROJECT_REMIND_DT) end as TRU_NEXT_DT
 from T_OE_PROJECTS P 
 left join T_OE_USERS UC on P.PROJECT_CONTACT_IDX=UC.USER_IDX and UC.ACT_IND=1 
 left join T_OE_USERS UC1 on COALESCE(P.MODIFY_USERIDX, P.CREATE_USERIDX) = UC1.USER_IDX
 where P.PROJ_STATUS <> 'Inactive'
 and isnull(P.PROJECT_REMIND_FREQ,'') <> 'N'
 and  (P.[PROJECT_REMIND_DT] is not null or DATEADD(month, 6, COALESCE(P.MODIFY_DT, P.CREATE_DT)) < GetDate());
